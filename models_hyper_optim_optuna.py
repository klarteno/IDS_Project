    ...
    def objective(trial: optuna.Trial):
        ...
        params = {
            ...
            "scheduler_iterations_restart": trial.suggest_int(
                "scheduler_iterations_restart",
                low=models_ops.dataInputParams.batch_size * 20,
                high=models_ops.dataInputParams.batch_size * 30,
                log=True,
            ),
            "scheduler_minimum_learning_rate": trial.suggest_float(
                "scheduler_minimum_learning_rate", 1e-5, 9e-1, log=True
            ),
        }
        ...
        scheduler_learning: torch.optim.lr_scheduler._LRScheduler = getattr(
            ...
            optimizer=optimizer,
            T_0=params["scheduler_iterations_restart"],
            eta_min=params["scheduler_minimum_learning_rate"],
        )

        models_trainning.setParameters(
            net_model, optimizer, scheduler_learning, trial_optimisation=trial
        )
        ...
        return models_trainning.get_evaluation_result(
            accuracy_trainning, loss_training, f1_score_trainning
        )
