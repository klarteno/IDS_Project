    ...
    def set_model_params_from(self, trial: optuna.Trial, model_name: ModelsNames):
        ...
        if model_name.value == ModelsNames.MLP.value:
            ...
            for i in range(mlp_trial_params.number_layers):
                num_units_l = trial.suggest_int(
                    self.num_units_l + "{}".format(i), 25, 256, step=3
                )
        ...
        elif (model_name.value is ModelsNames.CNN_BI_LSTM.value) or (
            model_name.value is ModelsNames.CNN_BI_GRU.value
        ):
            cnn_birnn_trial_params = CnnBiRnnTrialParams()

            cnn_birnn_trial_params.number_layers = no_layers = trial.suggest_int(
                self.n_layers, 2, 4
            )
            ...
            for i in range(no_layers):
                num_units_l = trial.suggest_int(
                    self.num_units_l + "{}".format(i), 25, 88, step=3
                )
                no_strides = trial.suggest_int(
                    self.no_strides_l + "{}".format(i), 1, 7, step=2
                )

                cnn_birnn_trial_params.stack_layers.append((num_units_l, no_strides))

            rnn_drop_procentages = trial.suggest_float(
                self.rnn_drop_procentages + "{}".format(i + 1), 0.1, 0.5, log=True
            )
    ...
    def set_model_params(self, model_name: ModelsNames, args: dict):
        if model_name.value == ModelsNames.MLP.value:
            ...
            for i in range(mlp_trial_params.number_layers):
                ...
                mlp_trial_params.stack_layers.append(
                    (args[num_units_l], args[dropout_l])
                )
            ...
            for i in range(cnn_birnn_trial_params.number_layers):
                ...
                cnn_birnn_trial_params.stack_layers.append(
                    (args[num_unit], args[no_stride])
                )

            cnn_birnn_trial_params.rnn_drop_procentages = args[
                self.rnn_drop_procentages + "{}".format(i + 1)
            ]
