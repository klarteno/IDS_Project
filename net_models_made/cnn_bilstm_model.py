...
class CNN_BILSTM(nn.Module):
    ...
    def forward(self, inputs, hidden=None):
        ...
        h0 = torch.zeros(self.n_layers * 2, out.size(0), self.hidden_size_lstm).to(
            DEVICE
        )
        c0 = torch.zeros(self.n_layers * 2, out.size(0), self.hidden_size_lstm).to(
            DEVICE
        )
...
def test_CNN_BILSTM_model(model, test_loader):
    ...
    model.to(DEVICE)
    # torch.nn.Module.train() -> Sets the module in evaluation mode.
    model.eval()
